...
app.post("/api/vocabulary",requireKey,async(req,res)=>{try{
  ...
  const targets=[...reviewTargets,...newTargets].filter(x =>
    /^[A-Za-z]+$/.test(String(x.word || "").trim())
  );
...
EVERY target is exactly ONE English word. Never convert it into a phrase, phrasal verb, idiom, collocation, or multi-word expression.
