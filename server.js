...
function validateVocabCandidate({rawQuestion,mode,expectedSource,recentSet,eligibleWeakSet,cooldownSet,selectedSet}){
  try{
    ...
    if(!word||!key)throw new Error("word is empty");if(selectedSet.has(key))throw new Error(`duplicate in current set: ${word}`);if(options.length!==4||new Set(options.map(x=>x.toLowerCase())).size!==4)throw new Error("invalid options");if(ai===null)throw new Error("invalid answer_index");
...
app.post("/api/vocabulary",requireKey,async(req,res)=>{try{
  const level=String(req.body.level||"B1"),topic=String(req.body.topic||"Daily conversation"),mode=String(req.body.mode||"en-ja");if(!["en-ja","ja-en","blank"].includes(mode))throw new Error("Vocabularyの出題形式が不正です。");const count=Math.max(5,Math.min(15,Number(req.body.count)||10));const recentWords=Array.isArray(req.body.recentWords)?req.body.recentWords.map(x=>String(x||"").trim()).filter(Boolean).slice(-200):[];const weakWords=Array.isArray(req.body.weakWords)?req.body.weakWords.map(x=>({word:String(x?.word||"").trim(),meaning_ja:String(x?.meaning_ja||"").trim(),count:Number(x?.count||1)})).filter(x=>x.word).slice(0,40):[];
  const cooldownSet=new Set(recentWords.slice(-30).map(normalizeWord));const eligibleWeak=weakWords.filter(x=>!cooldownSet.has(normalizeWord(x.word))).sort((a,b)=>b.count-a.count);const eligibleWeakSet=new Set(eligibleWeak.map(x=>normalizeWord(x.word)));const recentSet=new Set(recentWords.map(normalizeWord));const desiredReview=Math.min(Math.round(count*.2),eligibleWeak.length);const selected=[],selectedSet=new Set();
  const modeRule=mode==="blank"?`MODE blank: prompt is one natural English sentence with exactly one _____; options are four English words/phrases; word is correct option; meaning_ja Japanese; context empty.`:mode==="ja-en"?`MODE ja-en: prompt is Japanese meaning only; options are four English words/phrases; correct option exactly equals word; meaning_ja Japanese.`:`MODE en-ja: prompt is one English word/phrase exactly equal to word; options are four Japanese meanings; meaning_ja Japanese.`;
  async function candidates(source,needed){const n=Math.min(15,Math.max(5,needed+3));const recentText=recentWords.join(", ")||"(none)";const weakText=eligibleWeak.map(x=>`${x.word} (${x.meaning_ja})`).join(", ")||"(none)";const selectedText=Array.from(selectedSet).join(", ")||"(none)";const src=source==="review"?`Choose targets ONLY from eligible weak words: ${weakText}. Do not use already selected: ${selectedText}.`:`Choose NEW useful CEFR ${level} vocabulary for topic ${topic}. Do not use recent words: ${recentText}. Do not use already selected: ${selectedText}. Avoid trivial inflection/singular/plural workarounds.`;const p=`Create ${n} candidate vocabulary questions. ${modeRule} ${src} Return ONLY JSON {"questions":[{"prompt":"...","context":"...","options":["...","...","...","..."],"answer_index":0,"word":"...","meaning_ja":"...","explanation_ja":"..."}]}. Exactly ${n} candidates, 4 options each, no markdown.`;const d=await generateJson(p);return Array.isArray(d.questions)?d.questions:[];}
  function accept(arr,source,max){let a=0;for(const raw of arr){if(a>=max)break;const r=validateVocabCandidate({rawQuestion:raw,mode,expectedSource:source,recentSet,eligibleWeakSet,cooldownSet,selectedSet});if(!r.ok){console.log(`Vocabulary candidate rejected (${source}):`,r.reason);continue;}selected.push(r.question);selectedSet.add(normalizeWord(r.question.word));a++;}return a;}
  let remReview=desiredReview;for(let i=0;i<3&&remReview>0;i++){try{remReview-=accept(await candidates("review",remReview),"review",remReview);}catch(e){console.warn(e.message);}}
  let remNew=count-selected.length;for(let i=0;i<5&&remNew>0;i++){try{remNew-=accept(await candidates("new",remNew),"new",remNew);}catch(e){console.warn(e.message);}}
  if(selected.length<count)throw new Error(`問題を十分に生成できませんでした。${selected.length}/${count}問まで作成できました。もう一度お試しください。`);const final=selected.slice(0,count);for(let i=final.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[final[i],final[j]]=[final[j],final[i]];}res.json({questions:final});
}catch(e){console.error(e);res.status(500).json({error:e.message||"単語問題の作成に失敗しました。"});}});
