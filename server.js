...
app.post("/api/vocabulary",requireKey,async(req,res)=>{try{
  ...
  async function candidates(source,needed){const n=Math.min(15,Math.max(5,needed+3));const recentText=recentWords.join(", ")||"(none)";const masteredText=masteredWords.join(", ")||"(none)";const weakText=eligibleWeak.map(x=>`${x.word} (${x.meaning_ja})`).join(", ")||"(none)";const selectedText=Array.from(selectedSet).join(", ")||"(none)";const src=source==="review"?`Choose targets ONLY from eligible weak words: ${weakText}. Do not use already selected: ${selectedText}.`:`Choose NEW useful CEFR ${level} vocabulary for topic ${topic}. Do not use recent words: ${recentText}. NEVER use mastered words: ${masteredText}. Do not use already selected: ${selectedText}. Avoid trivial inflection/singular/plural workarounds.`;const p=`Create ${n} candidate vocabulary questions. ${modeRule} ${src} Return ONLY JSON {"questions":[{"prompt":"...","context":"...","options":["...","...","...","..."],"answer_index":0,"word":"...","meaning_ja":"...","explanation_ja":"..."}]}. Exactly ${n} candidates, 4 options each, no markdown.`;const d=await generateJson(p);return Array.isArray(d.questions)?d.questions:[];}
  function accept(arr,source,max){let a=0;for(const raw of arr){if(a>=max)break;const r=validateVocabCandidate({rawQuestion:raw,mode,expectedSource:source,recentSet,eligibleWeakSet,cooldownSet,selectedSet,masteredSet});if(!r.ok){console.log(`Vocabulary candidate rejected (${source}):`,r.reason);continue;}selected.push(r.question);selectedSet.add(normalizeWord(r.question.word));a++;}return a;}
